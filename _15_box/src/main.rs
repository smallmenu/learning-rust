...
fn main() {
    ...
    println!("Boxed box occupies {} bytes in the stack", size_of_val(&box_in_a_box));

    // 解引用，移除掉一层装箱，输出 16 bytes
    let unboxed_point: Point = *boxed_point;
    println!("Unboxed point occupies {} bytes in the stack", size_of_val(&unboxed_point));
}
